...
**Problem:** Default AI models may not be accessible (Groq's `llama-3.3-70b-versatile` returned 404 on a real run). Users can override via `PRISM_AI_MODEL` env var, but there's no CLI to set/view the model, no per-provider model storage, and error messages don't name the provider or model that failed.
...
Storage pattern: `~/.config/prism/<providerId>-model` (same as `<providerId>-key` but for model name).

Lookup order for `getModel`:
1. `PRISM_AI_MODEL` env var (global override, backwards-compatible)
2. `~/.config/prism/<providerId>-model` file
...
```
prism config set-model <provider> <model>
```

Validates provider name (anthropic, openai, gemini, groq, custom). Writes to `~/.config/prism/<provider>-model`.
